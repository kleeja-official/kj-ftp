...
//        [
    ...
    public function uploadTypeFile($fieldNumber, $current_uploading_folder, $current_user_id)
    {
        ...
        $fileInfo['originalFileName'] = isset($_FILES['file_' . $fieldNumber . '_']['name'])
            ? htmlspecialchars(str_replace([';',','], '', $_FILES['file_' . $fieldNumber . '_']['name']))
            : '';
    ...
    public function generateThumbnail($source_path, $ext, $saveToThisFtpPath)
    {
        global $config;
    ...
    public function generateWatermark($localFile, $ext)
    {
        helper_watermark(
            $localFile,
            $ext
    ...
    public function generate_htaccess($folder)
    {
